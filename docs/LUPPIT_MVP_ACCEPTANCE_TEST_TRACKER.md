...
| [x]  | VITAL-06 | P0  | Seller account creation                | On the second account choose Seller and complete phone OTP, legal acceptance, and Didit.                                                    | Exactly one seller profile/onboarding record exists and resumes at the correct setup step.                                           | PASS   | Seller setup works. The error shown when reusing the buyer email does not clearly explain that the email is already in use or whether a separate business email is required. The RNP field also lacks a short explanation.                                                    | BUG-001, BUG-002 |
| [x]  | VITAL-07 | P0  | Seller email and business submission   | Verify seller email, add required RNP/evidence files, and submit business verification.                                                     | One valid application becomes Pending/In review; evidence remains private; the restricted state is honest.                           | PASS   |                                                                                                                                                                                                                                                                               |                  |
...
| [x]  | VITAL-10 | P1  | Every core screen is reachable         | Under both roles tap every bottom tab. From Profile open Notifications, Help, Settings, Terms, Privacy, and Business where shown. Use Back. | Every configured screen opens with a deliberate data/empty state and returns without crash, blank screen, wrong role, or route loop. | PASS   | Core screens work. Help would benefit from more useful questions. Back from Business Settings incorrectly returns to Home instead of Settings.                                                                                                                                | BUG-003, BUG-004 |
| [x]  | VITAL-11 | P0  | Buyer AI understands intent            | As Buyer A describe a realistic item, answer follow-ups, then correct one important earlier detail.                                         | One continuous draft is used; questions are relevant; the correction replaces stale information.                                     | FAIL   | Tester result was partially correct. The keyboard return/arrow key sends the message instead of inserting a line break; sending should occur only through the green button. Switching apps interrupts the AI request, and the message has no resend action beside Copy/Share. | BUG-005, BUG-006 |
...
| [x]  | VITAL-17 | P0  | Pre-offer behavior is correct          | Open the new conversation as buyer and seller and inspect available actions/composer.                                                       | Request summary/title is visible, but neither participant can message before an offer exists.                                        | PASS   | The tested pre-offer behavior works. Sharing produces a message, but it is not a clickable link in WhatsApp or other destinations, so opening the request from the share remains unverified.                                                                                  | BUG-007          |
| [x]  | VITAL-18 | P0  | Seller AI builds a usable offer        | Choose Offer, use multiple turns, attach a real product photo, enter price/currency and shipping, then correct one detail.                  | One draft is retained; the correction sticks; valid photo and fulfillment values appear in the review.                               | FAIL   | Tester result was partially correct. Offer-draft messages are stored in `offer_draft_message`, but they are not restored when the seller leaves and reopens the assistant. There is also no action to discard the draft.                                                      | BUG-008, BUG-009 |

Section result: **IN PROGRESS — 15 PASS / 2 FAIL / 1 NOT RECORDED**<br>
Observations: Core account creation, onboarding, request creation, discovery, and pre-offer behavior passed. VITAL-11 and VITAL-18 were partially correct and are recorded as failures until their defects are retested. VITAL-09 has no recorded result.<br>
Open bugs/blockers: BUG-001 through BUG-009 and BUG-018 are implemented and awaiting manual retest. BUG-007 still needs the production Android signing SHA-256 before Android App Links can be verified.<br>
...
| [x] | VITAL-21 | P0 | Buyer can evaluate seller | As buyer open the offer and seller business profile. | Price, currency, photo, methods, business name, location, categories, and rating agree; no private business data leaks. | FAIL | Tester result was partially correct. Buyers can reach the business only through the three-dot menu and **Mostrar negocio**; tapping the business name above a message does nothing. Sellers also have no way to open the buyer profile for evaluation. | BUG-010, BUG-011 |
| [x] | VITAL-22 | P0 | Messaging and realtime work | While `OFFER_MADE`, exchange text and one valid image both ways. Background one device, send another message, then return. | Each logical message appears once in order; images open; unread/preview update; backgrounded device catches up. | FAIL | Tester result was partially correct. Sending text with three images made the chat unstable: the message duplicated, images changed into a carousel and then separated again, the duplicate later disappeared, and a maximum-image-size error appeared. | BUG-012 |
| [x] | VITAL-23 | P1 | In-app notification behavior works | Trigger an offer/message notification, open Notifications, tap it, then read/dismiss it. | It belongs to the active profile, opens the correct request/chat, and updates unread state consistently. | FAIL | The notification center, navigation, and read/dismiss behavior work, but sending an actual chat message does not create a notification. | BUG-013 |
...
| [x] | VITAL-25 | P0 | Seller commits to fulfill | Seller uses Concretar/confirm-sale and confirms. | Conversation becomes `SELLER_ACCEPTED` once; both actors see the correct next action/deadline and agreed offer. | PASS | Functional behavior passed. The **Retiro pendiente** status card has unclear copy and visual structure and should be redesigned. | BUG-014 |
...
| [x] | VITAL-28 | P0 | Ratings work both ways | Buyer rates seller, then seller rates buyer. Refresh summaries and try to rate again. | Each direction saves once; correct summaries update; each actor's rating action disappears after use. | PASS | Rating works, but the available rating-tag chips are not useful enough and should be revised in the database configuration and seed data. | BUG-015 |
| [x] | VITAL-29 | P0 | Completed state persists | Force-close both apps and reopen Home, Offers, Chats, request detail, and conversation. | Every screen agrees the transaction is finalized; no state rolls back, duplicates, or disappears. | PASS | Completed state persists correctly. Finalized requests/offers remain mixed into the buyer and seller active Home/Offers surfaces; they should have a separate completed-history space. | BUG-016 |
...
| [x] | VITAL-31 | P0 | Pickup commitment is distinct | Seller taps Concretar on the pickup order; buyer inspects the conversation. | State becomes `SELLER_ACCEPTED`; pickup-specific copy appears; no shipping action/code is incorrectly used yet. | PASS | | |
| [x] | VITAL-32 | P0 | Buyer can generate pickup code | At pickup time buyer chooses Generate pickup code and checks the verified inbox. | One purpose-specific 4-digit code is emailed to the buyer, expires as configured, and is never shown to seller. | PASS | | |
...
Section result: **TESTED — 16 PASS / 3 FAIL**<br>
Observations: Offer publication, acceptance, shipping, pickup, ratings, profile editing, and deletion completed successfully. Failures remain in business/profile discovery, multi-image messaging stability, and message-triggered notifications.<br>
Open bugs/blockers: BUG-010 through BUG-017 are implemented and awaiting manual retest.<br>
Tester/sign-off/date:
...
### Live remediation and retest queue — 2026-08-22
...
| --- | --- | --- | --- |
| BUG-001 | Ready for retest | Seller email conflict now says the address is already in use and that each profile needs a different email. | Rerun the email-conflict part of VITAL-06; verify the existing profile is unchanged and the seller can recover with a different email. |
| BUG-002 | Ready for retest | Business verification identifies RNP as the SICOP Registro Nacional de Proveedores and explains the certification number/evidence. | Rerun the RNP portion of VITAL-06/07 and confirm the label, helper text, and screen-reader hint are understandable. |
| BUG-003 | Ready for retest | The DB/seed now provides ten ordered MVP FAQs for both roles. | Rerun the Help portion of VITAL-10 as buyer and seller; open every FAQ and verify useful, nonblank answers. |
| BUG-004 | Ready for retest | Detail screens now use a nested stack, preserving detail-to-detail Back history. | Rerun Settings → Business Settings → Back from VITAL-10; it must return to Settings, not Home. |
| BUG-005 | Ready for retest | Buyer AI disables send-on-return; the green send button remains the send action. | Rerun VITAL-11 with a multi-line request; Return must add a line break and one green-button tap must send once. |
| BUG-006 | Ready for retest | Failed/interrupted buyer-AI turns expose Retry and reuse the exact request identity; the Edge Function stores durable replay results and serializes concurrent retries. | Rerun VITAL-11 by backgrounding during a normal and review→continue turn; Retry must recover once without a duplicate draft/request or duplicate assistant turn. |
...
| BUG-008 | Ready for retest | Seller offer assistant restores the latest owned active draft and its saved conversation; first-turn races reuse or safely reject a duplicate draft. | Rerun VITAL-18, leave after several turns, reopen, and confirm one draft, complete history, images, summary, and corrections are restored. |
| BUG-009 | Ready for retest | Seller drafts have a confirmed Discard action that soft-cancels the active draft. | Rerun VITAL-18, cancel the confirmation once, then discard; reopen and confirm the canceled draft is not restored and no offer was published. |
| BUG-010 | Ready for retest | The counterpart business name in buyer chat is an accessible link to the existing seller-business screen. | Rerun buyer VITAL-21 and tap the displayed seller name; verify the same safe business data and a working Back path. |
| BUG-011 | Ready for retest | Seller chat links to a conversation-scoped buyer profile exposing only name, image, rating, and rating count. | Rerun seller VITAL-21; verify access from the buyer name, exact safe fields, no private data, outsider denial, and Back. |
| BUG-012 | Ready for retest | Text plus up to ten images is preflighted, moderated, stored, and reconciled as one idempotent atomic message group with stable ordering/layout. | Rerun VITAL-22 with text + three images on two live devices, then retry once after an interrupted response; one stable group must appear, with no orphan files or duplicates. |
...
| BUG-014 | Ready for retest | Pickup status cards now use actor-specific next-step copy, DB-provided icons, and stronger visual hierarchy while shipping copy remains unchanged. | Rerun VITAL-25 for shipping and VITAL-31/32 for pickup on both roles; shipping must never say retiro and each pickup actor must know the next action. |
| BUG-015 | Ready for retest | Buyer/seller rating chips now use role-specific marketplace feedback tags in migration and seed configuration. | Rerun VITAL-28 both ways; verify all tags, multi-select/deselect, one submission per actor, refreshed summaries, and no duplicate rating. |
| BUG-016 | Ready for retest | Buyer Home and seller Offers separate History from active work; a finalized item remains actionable until that viewer rates, then moves, while other terminal seller outcomes remain in History. | Rerun VITAL-28/29 before and after each actor rates; verify exactly one item moves to History, persists after relaunch, and still opens its detail/chat. |
| BUG-017 | Ready for retest | Authoritative profile refresh now applies the full profile-switch boundary and routes Home when the active profile disappears. | Rerun VITAL-35 deleting either secondary role; verify immediate remaining-role Home, isolated tabs/data/unread state, retained session, and correct relaunch. |
...
| ---------------------------------------------- | ---------------: | ---: | ---: | ------: | --: | --------------------: | ---------: | --------------------------------------- | ---------- |
| **Phase 1A.1 onboarding through offer draft**  |           **18** |   15 |    2 |         |     |                     0 |          3 | **IN PROGRESS — 1 result not recorded** |            |
| **Phase 1A.2 offer through deletion**          |           **19** |   16 |    3 |         |     |                     0 |          2 | **TESTED — FAILURES OPEN**              |            |
| **Phase 1B accounts/profiles/screens/queries** |           **18** |      |      |         |     |                       |            | **NOT TESTED**                          |            |
| **Phase 1B conversation/request states**       |           **12** |      |      |         |     |                       |            | **NOT TESTED**                          |            |
| **Phase 1 total / gate**                       |           **67** |   31 |    5 |         |     |                     0 |          4 | **STOP — IN PROGRESS**                  |            |
